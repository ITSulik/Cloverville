...
public class MemberService {
  ...
  private static final Pattern VALID_NAME = Pattern.compile("^[a-zA-Z0-9 ]+$"); // letters, numbers, spaces only
  ...
  private void validateMember(Member m) {
    ...
    if (m.getPersonalPoints() < 0) throw new IllegalArgumentException("Points cannot be negative");
    if (m.getTotalTasksCompleted() < 0) throw new IllegalArgumentException("Total tasks cannot be negative");
  }
  ...
  private String generateUniqueName(String baseName) {
    ...
    for (Member m : members.values()) existing.add(m.getName());
  ...
  public List<Member> getByName(String name) {
    if (name == null || name.isBlank()) throw new IllegalArgumentException("Search name cannot be empty");
    List<Member> result = new ArrayList<>();
    String lower = name.toLowerCase();
    for (Member m : members.values()) {
      if (m.getName().toLowerCase().contains(lower)) result.add(m);
    }
    return result;
  }
  ...
  public void resetAllPoints() {
    for (Member m : members.values()) {
      m.setPoints(10); // reset all points to 10
    }
    save();
  }
  ...
  public void applyWeeklyBonusAndReset(SettingsService settingsService) {
    for (Member m : members.values()) {
      ...
      m.setTotalTasksCompleted(0); // reset weekly tasks
    }
    save();
